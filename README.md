...
当前版本：**1.3.3**（见 `packaging/version.txt`）
...
运行 `release\SelectionTranslation-Setup-1.3.3.exe`（需自行 [`打包`](#打包) 生成）。
...
└── release/                      # 打包产出（git 忽略）
    ...
    └── SelectionTranslation-Setup-1.3.3.exe
```
...
# 指定版本
.\packaging\build.ps1 -Version 1.3.3
...
| `release\SelectionTranslation-Setup-1.3.3.exe` | 安装包 |
