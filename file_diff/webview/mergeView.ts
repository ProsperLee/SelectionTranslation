/**
 * 三栏合并视图（左 / 结果 / 右）与 gutter 色带、接受/忽略按钮。
 *
 * 本地壳扩展：栏目标题可挂上传按钮（onSideUpload），用于加载文件后
 * 由外层重新 render(payload)。
 */
...
export class MergeView {
  ...
  /** Standalone harness: user clicked upload on a side pane header. */
  public onSideUpload?: (side: "ours" | "theirs") => void;
  ...
  private build(payload: MergeInitPayload): void {
    ...
    const leftBody = this.addPane(grid, 1, payload.oursLabel, true, "ours");
    this.gutterA = this.addGutter(grid, 2, "a");
    const resultBody = this.addPane(grid, 3, "结果", false);
    this.gutterB = this.addGutter(grid, 4, "b");
    const rightBody = this.addPane(grid, 5, payload.theirsLabel, true, "theirs");
  ...
  private addPane(
    ...
    if (uploadSide) {
      ...
      uploadBtn.className = "jb-pane-upload";
      uploadBtn.title = uploadSide === "ours" ? "上传左侧文件" : "上传右侧文件";
      uploadBtn.appendChild(iconElement(uploadIcon));
      uploadBtn.addEventListener("click", (event) => {
        event.stopPropagation();
        this.onSideUpload?.(uploadSide);
      });
      title.appendChild(uploadBtn);
    }
  ...
  private installSyncScroll(): void {
    for (const editor of this.editors) {
      const sub = editor.onDidScrollChange((event) => {
