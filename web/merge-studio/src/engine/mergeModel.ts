/**
 * 三方合并模型：base / ours / theirs → 变更块列表。
 *
 * 算法概要：
 * 1. 分别对 base↔ours、base↔theirs 做行 diff
 * 2. 按 base 坐标聚类重叠/相邻变更
 * 3. 仅一侧改动 → 可自动合并；两侧重叠 → conflict（内容相同则为 both-same）
 */
...
/**
 * 由 base / ours / theirs 构建合并模型（见文件头算法说明）。
 */
