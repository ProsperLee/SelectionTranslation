...
function withDefaultTheme(appValue: AppValue): AppValue {
  const mode = appValue.theme?.themeColorMode;
  if (mode && mode !== ThemeColorMode.default) {
    return appValue;
  }
  return { ...appValue, theme: DEFAULT_THEME };
}
...
export function App() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (storedData) {
        const appValue = withDefaultTheme(storedData as AppValue);
        setValue(appValue);
